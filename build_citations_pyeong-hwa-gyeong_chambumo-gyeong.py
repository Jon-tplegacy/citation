"""
Build reverse citation index for tplegacy.net scripture entries
(the two books: Pyeong Hwa Gyeong and Chambumo Gyeong).

Scans all posts tagged 'pyeong-hwa-gyeong' or 'chambumo-gyeong' via the
Ghost Content API, extracts internal <a> links with three contexts:
...
Inverts the forward map to produce a reverse citation index, saved as
citations-pyeong-hwa-gyeong_chambumo-gyeong.json sorted for stable diffs.
"""
...
OUTPUT_FILE = "citations-pyeong-hwa-gyeong_chambumo-gyeong.json"

CONTEXT_WEIGHT = {"verbatim": 3, "further_reading": 1, "mentioned": 2}
FURTHER_READING_HEADINGS = ("further reading", "key texts")


def fetch_all_scripture_posts():
    """Paginate through Ghost Content API for posts tagged with either book."""
    posts = []
    page = 1
    while True:
        r = requests.get(
            ...
            params={
                "key": API_KEY,
                # comma inside [] = "has any of these tags" (OR)
                "filter": "tag:[pyeong-hwa-gyeong,chambumo-gyeong]",
                "limit": "50",
                "page": str(page),
...
def main():
    print(f"Fetching scripture posts from {GHOST_URL} ...")
    posts = fetch_all_scripture_posts()
